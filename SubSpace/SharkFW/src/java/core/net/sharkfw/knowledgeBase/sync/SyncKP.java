...
/**
 * The SyncKP realizes a KnowledgePort that tries to propagate all changes in
 * the underlying knowledge base to all known peers (peers that are in the
 * knowledge base). That way a synchronization between this and every other Sync
 * KP happens.
 *
 * The identification of a Sync KP happens with a semantic tag with the subject
 * identifier "SharkKP_synchronization", so this subject identifier may not be
 * used in a knowledge base that is assigned to a Sync KP.
 *
 * The identification of context points that need to be sent to other peers uses
 * the time when a peer was last met and retrieves all context points from the
 * knowledge base that were updated since. That means when knowledge was sent to
 * a peer once he will never receive that knowledge again unless the context
 * point was updated in the meantine. To explicitly send all knowledge again to
 * a single peer or even send all knowledge again to all peers, there is the
 * syncAllKnowledge method.
 *
 * @author simon
 */
public class SyncKP extends KnowledgePort implements KnowledgeBaseListener
{
    ...
    private FragmentationParameter _peersFP;
    ...
    private void initialize(SharkEngine engine, SyncKB kb, SharkCS interest, PeerSTSet remotePeers, int retryTimeout) throws SharkKBException
    {
        ...
        _retryTimeout = retryTimeout * 1000;
    ...
    private boolean addIdentifierTag() throws SharkKBException
    {
        boolean tagAdded = false;
        SharkCS contextSpace = getInterest();
        if (contextSpace != null && contextSpace.getTopics() != null)
        {
            STSet topics = contextSpace.getTopics();
            try
            {
                topics.createSemanticTag(SYNCHRONIZATION_NAME, SYNCHRONIZATION_NAME);
            } catch (SharkKBException e)
            {
                L.e("Tag SharkKP_synchronization which is used by SyncKP already exists!");
                throw e;
            }
            tagAdded = true;
        }
    ...
    /**
     * This SyncKP will keep the assigned Knowledge Base synchronized with all
     * peers. Fragmentation parameter can be set via the setTopicsFP or
     * setPeersFP method.
     *
     * @param engine
     * @param kb The underlying kb that will synchronize changes made to it with
     * others
    ...
    public SyncKP(SharkEngine engine, SyncKB kb, int retryTimeout) throws SharkKBException
    {
        ...
        initialize(engine, kb, syncInterest, peersToSyncWith, retryTimeout);
    }
    ...
    /**
     * Setter for the topics fragmentation parameter that will allow the user of
     * the SyncKB to synchronize non-contextualized data.
     *
     * @param topicsFP
     */
    ...
    /**
     * Setter for the peer fragmentation parameter that will allow the user of
     * the SyncKB to synchronize non-contextualized data.
     *
     * @param peersFP
     */
    ...
    /**
     * Explicitly sync the entire knowledge base again with all peers. Will
     * cause huge traffic with big knowledge bases.
     */
    ...
    /**
     * Explicitly sync the entire knowledge base again with a peer. This can be
     * used when a new peer is added to the knowledge base to get her or him "up
     * to date".
     *
